...
export default async function EditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <main className="mx-auto min-h-screen max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <PageForm endpoint={`/api/pages/${page.id}`} submitLabel="Save changes" initialValues={{ title: page.title, slug: page.slug, bio: page.bio ?? '', avatarUrl: page.avatarUrl ?? '', logoUrl: page.logoUrl ?? '', backgroundType: (page.backgroundType as 'gradient' | 'image' | 'video') ?? 'gradient', backgroundValue: page.backgroundValue ?? '', foregroundMedia: page.foregroundMedia ?? '', customCss: page.customCss ?? '', seoTitle: page.seoTitle ?? '', seoDescription: page.seoDescription ?? '', themePreset: page.themePreset, links: page.links.map((link) => ({ id: link.id, label: link.label, url: link.url, icon: link.icon ?? '', linkType: link.linkType, isFeatured: link.isFeatured, sortOrder: link.sortOrder })) }} />
    </main>
  );
}